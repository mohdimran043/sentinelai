...
### 2. The console's generated types are not gated

Web CI exists — `.github/workflows/web-ci.yml` runs `npm ci`, `typecheck`,
`lint`, `test` and the production `build` on every push and pull request, as a
workflow separate from Python's `ci.yml` so neither can block the other.

What it does **not** check is that `web/src/api/engine.types.ts` still matches
`contracts/openapi/ai-engine.yaml`. That file is generated by `npm run gen:api`
and committed, so a change to the contract that nobody regenerates against
leaves the console typechecking cleanly against a schema the engine no longer
serves. This is not hypothetical: the camera-editing contract landed with the
generated types 197 lines behind it, and `tsc` stayed green until they were
regenerated. Run `npm run gen` whenever `contracts/` changes. The engine side of
the same contract *is* gated, by `ai-engine/tests/test_openapi_contract.py`.
...
### 5. VRAM measurement undershoots real peaks
...
### 6. `RtspSource.close()` can take up to ~20 s
