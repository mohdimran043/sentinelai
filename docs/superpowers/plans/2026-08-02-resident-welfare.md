...
**Call `install_httpx_log_redaction()` in the composition root.** Task 6 exposes it as an explicit named function precisely so it is not a hidden side effect of constructing an adapter — which means it is currently inert. httpx's own logger emits `HTTP Request: POST <url>` at INFO, and a webhook URL routinely embeds a credential (ntfy and Slack put tokens in the path). If nobody calls this, that token lands in the logs of every deployment that configures a webhook. Call it where the notifier is built, and call `remove_httpx_log_redaction()` on shutdown.
